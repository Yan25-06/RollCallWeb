...
- **Viết hoa:** dùng **sentence case** cho mọi nhãn, tiêu đề trang, tiêu đề card, tab và nút — "Lớp học", "Điểm danh", "Xếp lịch". KHÔNG Title Case ("Lớp Học"): tiếng Việt có dấu đọc Title Case chậm hơn hẳn. Ngoại lệ giữ nguyên: danh từ riêng (`Mock Test`, `Excel`, `Admin`, `Dashboard`) và tên thứ trong tuần (`Thứ Hai`, `Chủ Nhật`). `uppercase tracking-wide` chỉ dành cho nhãn stat nhỏ và header bảng.
- **Thông tin không được thua cuộc tranh chỗ:** trong container hẹp (card lịch, sidebar), thứ định danh phần tử (tên lớp, tiêu đề panel) phải có hàng riêng — không đặt cạnh badge/nút rồi để `truncate` ăn mất. Xem `ScheduleCard.jsx` và `StudentSidebar.jsx` làm mẫu. Mọi chỗ còn `truncate` phải kèm `title=`.
- **Không mã hóa thông tin bằng chấm màu trần.** Loại khóa hiện bằng badge có chữ (`DailyAgenda.jsx`, `ScheduleCard.jsx`); màu chỉ là kênh phụ.
