...
- `VITE_GOOGLE_SHEETS_API_KEY` (**tùy chọn**): cần cho tab "Tài Liệu" đọc giáo trình từ Google Sheets API (xem "Model giáo trình đọc trực tiếp từ Google Sheet" bên dưới). Thiếu biến này không làm app crash khi khởi động — chỉ `googleSheetCurriculumService.fetchCurriculum()` throw lỗi tiếng Việt khi tab đó cố tải dữ liệu.
...
- `ClassDetailPage` có các tab: Students, Attendance, Homework, MockTest. Tab Students: nút "＋ Thêm" mở `BulkEnrollPickerModal` (chọn nhiều HS chưa có trong lớp + học phí chung → ghi danh hàng loạt qua `enrollMany`); admin có thêm nút "Tạo mới" mở `EnrollmentModal` để tạo học sinh mới. Prop `initialTab` (default `'students'`) cho phép mở thẳng tab Attendance từ Dashboard. Truyền `scheduleTime={currentClass?.scheduleTime}` vào `AttendanceTab` và `HomeworkTab`; các tab tiếp tục truyền prop này vào `SessionModal` để tự điền giờ khi tạo buổi mới.
...
- **Trang "Giảng Dạy" (SchedulePage) — 3 tab:**
  ...
  - **Tab "Tài Liệu"** (`MaterialsTab.jsx`): giáo trình & tài liệu giảng dạy **dùng chung theo `courseType`**, **đọc trực tiếp từ Google Sheet** (nguồn chân lý là file Sheet, không phải DB CRUD — xem "Model giáo trình đọc trực tiếp từ Google Sheet" bên dưới để biết chi tiết service/kiến trúc). Toolbar: dropdown chọn `courseType` (`COURSE_TYPES`) + nút "Làm mới" (refetch, bỏ qua cache) + "Mở Sheet" (mở link Sheet tab mới) + admin có thêm "Cấu hình Sheet" (mở `SheetConfigModal.jsx`). **Layout master-detail giữ nguyên từ trước:** sidebar trái `CurriculumSidebar.jsx` (cây Tháng → Tuần → Buổi) + panel phải `SessionDetailPanel.jsx` (chi tiết buổi, tài liệu dạng hàng lớn, cả hàng là link khi có `url`) — nhưng giờ **luôn nhận `isAdmin={false}`** (read-only hoàn toàn, không còn nút Thêm/Sửa/Xóa vì nội dung sửa trực tiếp trên Sheet). State `selectedSessionId` tự chọn buổi đầu tiên khi đổi khóa/load xong; dưới breakpoint `lg` hai khối xếp dọc. Cache trong phiên theo `courseType` (`cacheRef`), có guard `requestIdRef` chống race condition khi đổi `courseType` nhanh trước khi fetch cũ trả về. 4 modal CRUD cũ (`MonthModal.jsx`/`SessionModal.jsx`/`MaterialModal.jsx`/`ImportCurriculumModal.jsx`) đã bị **xóa**.
...
## Model giáo trình dùng chung — DB CRUD CŨ (migration `20260704000001_create_curriculum.sql`) — ĐÃ THAY THẾ
- 3 bảng phân cấp **Tháng → Buổi → Tài liệu** (`curriculum_months`/`curriculum_sessions`/`curriculum_materials`, khóa theo `course_type`) và service `curriculumService.js` **vẫn còn trong DB/repo nhưng đã orphan** — không còn component nào import `curriculumService`. Migration `20260704000001` giữ nguyên để lưu lịch sử, không có migration drop. Model CRUD-qua-app này (admin biên soạn tháng/buổi/tài liệu ngay trong app qua `MonthModal`/`SessionModal`/`MaterialModal`, giáo viên xem read-only) đã bị thay thế hoàn toàn bởi model đọc trực tiếp từ Google Sheet bên dưới — cùng tinh thần với "Model tài liệu giảng dạy CŨ (`class_materials`...)" ở trên: giữ bảng cũ cho lịch sử, không xóa, ngừng dùng ở app layer.
- Các file UI CRUD cũ đã bị **xóa** hẳn (không chỉ orphan): `MonthModal.jsx`, `SessionModal.jsx` (trong `schedule/`, không phải `classes/SessionModal.jsx`), `MaterialModal.jsx`, `ImportCurriculumModal.jsx` (import từ Excel xuất từ Google Sheet — không còn cần vì giờ đọc thẳng từ Sheet qua API).
- **Vẫn còn dùng lại** (không phải orphan): `src/utils/curriculumImportParser.js` — parser thuần (không import xlsx/React) nhận diện dòng Tháng (`THÁNG n:`) / Tuần (`tuần n`) / Buổi, tách ô tài liệu theo nhãn `PPT`/`Handout`/`Đọc dịch`/`Dictation`/`Homework` (`parseMaterialsCell`); được **tái sử dụng** bởi `googleSheetCurriculumService` ở model mới (parse dữ liệu Sheet đọc qua API thay vì file `.xlsx` import thủ công). Test: `node scripts/test-curriculum-parser.mjs`. `src/components/schedule/useArmedDelete.js` cũng còn dùng (bởi `SessionDetailPanel`, dù hiện tại panel luôn render read-only nên hook không kích hoạt UI xóa).

## Model giáo trình đọc trực tiếp từ Google Sheet (migration `20260705000001_add_curriculum_sheets.sql`)
- Google Sheet là **nguồn chân lý** cho nội dung giáo trình — không còn CRUD tháng/buổi/tài liệu trong app. Mỗi loại khóa (`courseType`) trỏ tới đúng 1 file Google Sheet; sửa nội dung = sửa trực tiếp trên Sheet.
- **Bảng `curriculum_sheets`**: PK `course_type` (text) → `sheet_url` (text). RLS: mọi giáo viên đã đăng nhập SELECT được; chỉ admin INSERT/UPDATE/DELETE.
- **`src/services/curriculumSheetService.js`**: `getAll()` → map `{ courseType: sheetUrl }`; `upsert(courseType, sheetUrl)`; `remove(courseType)`.
- **`src/services/googleSheetCurriculumService.js`**: `fetchCurriculum(sheetUrl)` — gọi thẳng Google Sheets API v4 qua `fetch` (không dùng SDK), đọc key từ `import.meta.env.VITE_GOOGLE_SHEETS_API_KEY`, request chỉ `fields=sheets(data(rowData(values(formattedValue,hyperlink,textFormatRuns))))` (giảm payload so với full grid). Convert response qua `apiRowsToGrid` rồi parse bằng `parseCurriculumGrid` (tái dùng nguyên từ `curriculumImportParser.js`), gắn id qua `attachIds`. Trả `{ tree, warnings }`, `tree` cùng shape với `curriculumService.getByCourseType` cũ nên `CurriculumSidebar`/`SessionDetailPanel` dùng lại nguyên không đổi. Map lỗi HTTP 403/404/khác sang thông báo tiếng Việt (thiếu quyền chia sẻ Sheet, sai link, lỗi API...); thiếu `VITE_GOOGLE_SHEETS_API_KEY` → throw ngay từ đầu hàm.
- **`src/utils/googleSheetGrid.js`** (pure util, test bằng Node — `node scripts/test-googlesheet-grid.mjs`): `extractSpreadsheetId(url)`, `apiRowsToGrid(rowData)` (convert response Sheets API → grid 2D dùng được bởi parser cũ), `attachIds(months)`.
- **`src/components/schedule/SheetConfigModal.jsx`**: modal chỉ admin thấy — sửa link Sheet cho từng `courseType` trong `COURSE_TYPES`, gọi `curriculumSheetService.upsert/remove`.
- **`MaterialsTab.jsx` (viết lại hoàn toàn, read-only)**: nạp `sheetMap` (mọi GV đọc được) → fetch + cache trong phiên (theo `courseType`) cây giáo trình từ Sheet đang cấu hình. Toolbar: chọn `courseType` + "Làm mới" (bypass cache, refetch) + "Mở Sheet" (mở link Sheet tab mới) + admin có thêm "Cấu hình Sheet" (mở `SheetConfigModal`). Render `CurriculumSidebar`/`SessionDetailPanel` sẵn có với `isAdmin={false}` cố định (không còn nút Thêm/Sửa/Xóa). Guard `requestIdRef` chống race condition khi đổi `courseType` liên tục trước khi fetch cũ trả về kịp.
- Loại tài liệu + badge màu vẫn dùng `src/components/schedule/materialType.js` (`MATERIAL_TYPES`: PPT/Handout/Đọc dịch/Homework/Khác) — không đổi.
- `COURSE_TYPES` (`src/utils/courseTypes.js`) không đổi, vẫn dùng ở `ClassModal` và `MaterialsTab`.
